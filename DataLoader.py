...
class IEMOCAPDataset(Dataset):
    ...
    def __init__(self, dataset_path, vocab_path, mode='train'):
        ...
        self.videoIDs, self.videoSpeakers, self.videoLabels, self.videoText,        self.videoAudio, self.videoVisual, self.videoSentence, self.trainVid,        self.testVid = pickle.load(open(dataset_path, 'rb'), encoding='latin1')
        '''
    ...
    def __getitem__(self, index):
        vid = self.keys[index]
        return  self.videoSentence[vid],                torch.FloatTensor(self.utterance_len[vid]),                torch.FloatTensor(self.videoVisual[vid]),                torch.FloatTensor(self.videoAudio[vid]),                torch.FloatTensor([[1,0] if x=='M' else [0,1] for x in self.videoSpeakers[vid]]),                torch.FloatTensor([1]*len(self.videoLabels[vid])),                torch.LongTensor(self.videoLabels[vid]),                vid
...
class IEMOCAPPadCollate:
    ...
    def __call__(self, batch):
        ...
        return [self.pad_collate(dat[i]) if i==0 else                 pad_sequence(dat[i], True) if i < 7 else                 dat[i].tolist() for i in dat]
